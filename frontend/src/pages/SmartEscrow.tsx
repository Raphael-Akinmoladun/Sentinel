...
export default function SmartEscrow() {
  ...
  // Pre-fill ID if coming from Dashboard
  useEffect(() => {
    if (location.state?.shipmentId) {
      setVendorId(location.state.shipmentId);
    }
  }, [location.state]);
  ...
  const [productName, setProductName] = useState('Custom Order');
  ...
  const searchVendor = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.get(`${BACKEND_URL}/api/shipments/details/${vendorId}`);

      if (response.data.success) {
        const { data } = response.data;
        setVirtualAccount(data.virtualAccount);
        setProductName(data.productName);
        if (data.amount > 0) setAmount(data.amount.toString());
        setStatus('found');
      } else {
        setStatus('error');
      }
  ...
  const fundEscrow = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // NOTE: Using a placeholder token for the demo. In a real app, this would be from your login state.
      const response = await axios.post(`${BACKEND_URL}/api/payments/initiate`, {
        amount: parseFloat(amount),
        email: email,
        shipmentId: vendorId 
      }, {
        headers: { 'Authorization': 'Bearer DUMMY_BUYER_TOKEN' }
      });

      // 2. Redirect to Squad Checkout Page
            ...
            <div className="glass-panel animate-fade-in">
              ...
              <div style={{ background: 'rgba(255,255,255,0.5)', padding: '1rem', borderRadius: '12px', border: '1px solid rgba(0,0,0,0.05)', marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.25rem' }}>Supplier</p>
                    <p style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{virtualAccount?.account_name}</p>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.25rem' }}>Product</p>
                    <p style={{ fontWeight: 600, color: 'var(--accent-primary)' }}>{productName}</p>
                  </div>
                </div>
              </div>
